...
<body class="hello">
  <nav>
    <h3>Local Client</h3>
    <h4>Room: <span id="roomId">{roomId}</span></h4>
    <h4>UserId: <span id="userId"></span></h4>
    <h4>Observer Address: <span id="observerAddress">{observerAddress}</span></h4>
    <ul id="lablels">
    ...
    <div id="localMetrics" class="stats"></div>
  </nav>
...
export function makeRoomHtml(options: RoomHtmlOptions) {
  const result = roomTemplate
    ...
    .replace("{observerAddress}", options.observerAddress)
